...
class EVMApp(tk.Tk):
    ...
    def show_results(self):
        try:
            ...
            results_frame = ttk.Frame(results_window, padding="20 20 20 20")
            results_frame.pack(expand=True, fill=tk.BOTH)

            title_label = ttk.Label(results_frame, text="Voting Results", font=("Helvetica", 18, "bold"))
            title_label.pack(pady=(0, 20))

            for i, (party_name, votes) in enumerate(results):
                result_label = ttk.Label(results_frame, text=f"{party_name}: {votes} votes")
                result_label.pack()

            winner = results[0]
            winner_label = ttk.Label(results_frame, text=f"\nWinner: {winner[0]}\nVotes: {winner[1]}", font=("Helvetica", 14, "bold"))
